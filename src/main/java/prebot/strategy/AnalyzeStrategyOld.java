...
/// 봇 프로그램 설정
public class AnalyzeStrategyOld {
							...
							for(Unit gateway : Prebot.Broodwar.enemy().getUnits()){
								...
								if(gateway.getType() == UnitType.Protoss_Gateway|| gateway.getType() == UnitType.Protoss_Pylon){
									if(center.getDistance(gateway.getTilePosition()) < 600){
										RespondToStrategyOld.Instance().center_gateway= true;
									}
								}
							}
					...
					if(7000 <  Prebot.Broodwar.getFrameCount()){
						if(InformationManager.Instance().getNumUnits(UnitType.Protoss_Zealot,InformationManager.Instance().enemyPlayer)
							...
							if (StrategyManager.Instance().getCurrentStrategyException() == EnemyStrategyException.PROTOSSEXCEPTION_DOUBLENEXUS
								...
								RespondToStrategyOld.Instance().prepareDark = true;
							}
						}
					}
	...
	if(StrategyManager.Instance().getCurrentStrategyException()==EnemyStrategyException.PROTOSSEXCEPTION_DOUBLENEXUS||selectedSE==EnemyStrategyException.PROTOSSEXCEPTION_DOUBLENEXUS)
	{
		if (Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) >= 2
				&& UnitUtils.myFactoryUnitSupplyCount() > 64) {
			selectedSE = EnemyStrategyException.INIT;
		}
	}
	...
	if(RespondToStrategyOld.Instance().enemy_scout==false&&(InformationManager.Instance().getNumUnits(UnitType.Protoss_Scout,InformationManager.Instance().enemyPlayer)>=1))
	{
	...
	if(RespondToStrategyOld.Instance().enemy_shuttle==false&&(InformationManager.Instance().getNumUnits(UnitType.Protoss_Shuttle,InformationManager.Instance().enemyPlayer)>=1))
	{
		selectedSE = EnemyStrategyException.PROTOSSEXCEPTION_SHUTTLE;
	}

	if(RespondToStrategyOld.Instance().enemy_arbiter==false&&(InformationManager.Instance().getNumUnits(UnitType.Protoss_Arbiter,InformationManager.Instance().enemyPlayer)>=1||InformationManager.Instance().getNumUnits(UnitType.Protoss_Arbiter_Tribunal,InformationManager.Instance().enemyPlayer)>=1))
	{
	...
	if(RespondToStrategyOld.Instance().enemy_dark_templar==false&&(InformationManager.Instance().getNumUnits(UnitType.Protoss_Templar_Archives,InformationManager.Instance().enemyPlayer)>=1||InformationManager.Instance().getNumUnits(UnitType.Protoss_Citadel_of_Adun,InformationManager.Instance().enemyPlayer)>=1||InformationManager.Instance().getNumUnits(UnitType.Protoss_Dark_Templar,InformationManager.Instance().enemyPlayer)>=1))
	{

		selectedSE = EnemyStrategyException.PROTOSSEXCEPTION_DARK;
	}if(RespondToStrategyOld.Instance().enemy_dark_templar==false)
	{
	...
	if(selectedSE==EnemyStrategyException.PROTOSSEXCEPTION_DARK||StrategyManager.Instance().getCurrentStrategyException()==EnemyStrategyException.PROTOSSEXCEPTION_DARK)
	{
		...
		boolean mainBaseTurret = RespondToStrategyOld.Instance().mainBaseTurret;
		boolean firstChokeTurret = RespondToStrategyOld.Instance().firstChokeTurret;
	...
	private void AnalyzeVsTerran() {
		...
		if (RespondToStrategyOld.Instance().enemy_wraithcloak == false && InformationManager.Instance()
		...
		if (RespondToStrategyOld.Instance().enemy_wraithcloak == false) {
			for (Unit unit : Prebot.Broodwar.enemy().getUnits()) {
				if (unit.isVisible() && (!unit.isDetected() || unit.getOrder() == Order.Burrowing)) {
					selectedSE = EnemyStrategyException.TERRANEXCEPTION_WRAITHCLOAK;
				}
	...
	private void AnalyzeVsZerg() {
		...
		if (InformationManager.Instance().getNumUnits(UnitType.Zerg_Hydralisk_Den,
			...
			if (RespondToStrategyOld.Instance().enemy_lurker == false && cntLair >= 1) {
		...
		// lurker in 조건
		if (RespondToStrategyOld.Instance().enemy_lurker == false) {
			for (Unit unit : Prebot.Broodwar.enemy().getUnits()) {
