...
public class StargateAnalyser extends UnitAnalyser {
	...
	private void fastStargate() {
		...
		if (!found.isEmpty()) {
			...
			int fastStargateFrame = EnemyStrategy.PROTOSS_STARGATE.defaultTimeMap.time(UnitType.Protoss_Stargate, 30);
			int doubleStargateFrame = EnemyStrategy.PROTOSS_DOUBLE_CARRIER.defaultTimeMap.time(UnitType.Protoss_Stargate, 30);
