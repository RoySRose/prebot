	...
	private void fastFactory() {
		...
		if (!found.isEmpty()) {
			if (found.size() >= 2) { // 팩토리 2개 이상
				...
				int twoFacSecondFacFrame = EnemyStrategy.TERRAN_2FAC.defaultTimeMap.timeOfIndex(UnitType.Terran_Factory, 1, 20);
				...
				int firstBuildFrame = buildStartFrameDefaultJustBefore(found.get(0));
				int mechanicFirstBarrackFrame = EnemyStrategy.TERRAN_2FAC.defaultTimeMap.time(UnitType.Terran_Factory, 15);
			...
			int firstBuildFrame = EnemyStrategy.TERRAN_2FAC.defaultTimeMap.time(UnitType.Terran_Barracks, 15);
