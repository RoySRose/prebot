...
public class StrategyManager {
	...
	private void setCombatUnitRatio() {
		...
		if (currentStrategyException == EnemyStrategyException.INIT) {
			vultureratio = currentStrategy.vultureRatio;
			tankratio = currentStrategy.tankRatio;
			goliathratio = currentStrategy.goliathRatio;
			wgt = currentStrategy.weight;
		} else {
				...
				for (int i = 0; i < 4; i++) {
					if (Currentbool[i] == true) {
						...
						if (terranBio && i == 2 && Current[2].getTechType() == TechType.Spider_Mines) {
							if ((myFactoryUnitSupplyCount > 48 && Prebot.Broodwar.self().minerals() > 200 && Prebot.Broodwar.self().gas() > 150) || myFactoryUnitSupplyCount > 100) {
