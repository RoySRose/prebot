...
public class EnemyStrategy {
	...
	private static final int NO_MARINE = 0;
	private static final int TWO_MARINE = 2;
	private static final int FOUR_MARINE = 4;
	private static final int SIX_MARINE = 6;
	private static final int EIGHT_MARINE = 8;
	...
	// 시즈업
	private static final List<MetaType> TS = Arrays.asList(new MetaType(TechType.Tank_Siege_Mode));
	...
	// 속도업-시즈업-마인업
	private static final List<MetaType> VS_TS_VM = Arrays.asList(new MetaType(UpgradeType.Ion_Thrusters)
			, new MetaType(TechType.Tank_Siege_Mode)
			, new MetaType(TechType.Spider_Mines));
	...
	// 시즈업-마인업-속도업-골리앗업
	private static final List<MetaType> TS_VM_VS_GR = Arrays.asList(new MetaType(TechType.Tank_Siege_Mode)
			, new MetaType(TechType.Spider_Mines)
			, new MetaType(UpgradeType.Ion_Thrusters)
			, new MetaType(UpgradeType.Charon_Boosters));
	...
	public static enum Strategy {
		...
		PROTOSS_INIT(1, 1, 0, VM_TS_VS,
				SIX_MARINE, AddOnOption.IMMEDIATELY, ExpansionOption.ONE_FACTORY), // INIT DEFAULT
		
		PROTOSS_1GATE_CORE(PROTOSS_INIT), //
		
		PROTOSS_2GATE(6, 1, 0, VM_TS_VS,
				NO_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // 
		
		PROTOSS_DOUBLE(1, 1, 0, VM_TS_VS,
				NO_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.ONE_FACTORY), // camp=F_EXPANSION
		
		PROTOSS_FORGE(1, 1, 0, TS_VM_VS,
				NO_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.ONE_FACTORY), //
		// 넥서스가 소환되지 않는 경우 포톤러시를 의심해야 한다.
		
		PROTOSS_FORGE_DOUBLE(1, 1, 0, TS_VM_VS,
				NO_MARINE, AddOnOption.IMMEDIATELY, ExpansionOption.TWO_FACTORY), // camp=S_CHOKE, 공격실패시 F_EXPANSION
		...
		PROTOSS_MORE_GATE(1, 1, 0, TS_VM_VS,
				SIX_MARINE, AddOnOption.IMMEDIATELY, ExpansionOption.ONE_FACTORY), // camp=F_EXPANSION
		// + 위험종료 : PROTOSS_DEFAULT와 다르지 않으므로 바로종료 (TODO TBD: 정면이 위험하면 입구심시티)
		
		PROTOSS_ROBOTICS(5, 5, 2, VM_TS_VS,
				SIX_MARINE, AddOnOption.IMMEDIATELY, ExpansionOption.ONE_FACTORY), //
		// + WATCHER : 마인매설(+본진)
		...
		ZERG_INIT(2, 1, 2, VM_GR_TS_VS
				, SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // INIT DEFAULT, camp=S_CHOKE,
		
		ZERG_5DRONE(7, 1, 1, VS_VM_GR_TS
				, EIGHT_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=BASE, 벙커
		
		ZERG_9DRONE(2, 1, 2, VM_GR_TS_VS,
				EIGHT_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_CHOKE, 벙커
		
		ZERG_9DRONE_GAS(2, 1, 2, VS_VM_GR_TS,
				EIGHT_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // 마린=MORE, camp=F_CHOKE, 벙커
		
		ZERG_OVER_POOL(2, 1, 2, VM_GR_TS_VS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_CHOKE
		
		ZERG_OVER_POOL_GAS(2, 1, 2, VS_VM_GR_TS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_CHOKE
		
		ZERG_2HAT(2, 1, 2, VM_GR_TS_VS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=S_CHOKE, 벙커(공격)
		
		ZERG_2HAT_GAS(2, 1, 2, VM_GR_VS_TS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=S_CHOKE, 벙커(공격)

		// PHASE2 : PHASE1 종료 ~ PHASE2 에 대한 위험이 종료되는 시점 (camp가 F_EXPANSION으로 이동, 적 병력/다크, 아군 병력/터렛/컴셋 고려)
		ZERG_NO_LAIR_LING(3, 1, 0, VS_VM_GR_TS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_EXPANSION, 벙커
		// + CHECKER : 할당량 감소(거의 할당하지 않음)
		// + 위험종료 : BASE근처에 적이 없음. 벙커 완성. 벌처 일정량 이상 보유. 
		
		ZERG_NO_LAIR_HYDRA(1, 5, 0, VM_TS_VS_GR,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_EXPANSION, 벙커
		// + WATCHER : 마인매설(+촘촘하게)
		// + CHECKER : 할당량 감소(거의 할당하지 않음)
		// + 위험종료 : BASE근처에 적이 없음. 벙커 완성. 탱크 일정량 이상 보유.
		
		ZERG_LAIR_HYDRA_TECH(1, 5, 2, VM_TS_VS_GR,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_EXPANSION
		
		// + 위험종료 : BASE근처에 적이 없음. 포지션별 터렛 완성. 탱크 일정량 이상 보유.
		
		ZERG_LAIR_SPIRE_TECH(1, 1, 15, VM_GR_VS_TS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_EXPANSION
		// + 위험종료 : BASE근처에 적이 없음. 포지션별 터렛 완성. 골리앗 일정량 이상 보유.
		
		ZERG_LAIR_MIXTED_TECH(1, 3, 5, VM_GR_VS_TS,
				SIX_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY), // camp=F_EXPANSION
		// + 위험종료 : BASE근처에 적이 없음. 포지션별 터렛 완성. 골리앗 일정량 이상 보유.
		...
		TERRAN_INIT(1, 1, 0, TS,
				TWO_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_STARPORT),
		...
		TERRAN_2BARRACKS(2, 1, 0, VS_TS_VM,
				FOUR_MARINE, AddOnOption.VULTURE_FIRST, ExpansionOption.TWO_FACTORY),
		...
		// PHASE3 : PHASE2 종료 ~
		TERRAN_MECHANIC_DEFAULT(0, 5, 1, TS_VM_VS_GR),
		TERRAN_MECHANIC_VULTURE_TANK(TERRAN_MECHANIC_DEFAULT),
		TERRAN_MECHANIC_GOLIATH_TANK(TERRAN_MECHANIC_DEFAULT),
		TERRAN_MECHANIC_WRAITH_TANK(0, 3, 1, TS_VM_VS_GR),
		TERRAN_MECHANIC_BATTLE(0, 2, 1, TS_VM_VS_GR);
		...
		private Strategy(int vulture, int tank, int goliath, List<MetaType> upgrade) {
			...
			this.marineCount = SIX_MARINE;
			this.addOnOption = AddOnOption.VULTURE_FIRST;
			this.expansionOption = ExpansionOption.TWO_FACTORY;
		}

		private Strategy(int vulture, int tank, int goliath, List<MetaType> upgrade, int marineCount, AddOnOption addOnOption, ExpansionOption expansionOption) {
			this.ratio = FactoryRatio.ratio(vulture, tank, goliath);
			this.upgrade = upgrade;
