...
public class BuilderControlTower extends DefaultBuildableItem {
    ...
    public final boolean buildCondition(){
		...
		if (Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Wraith) >= 4 && StrategyIdea.wraithCount >= 6) {
			addControlTower = true;
		} else if (Prebot.Broodwar.self().allUnitCount(UnitType.Terran_Science_Facility) >= 1) {
			addControlTower = true;
		}
