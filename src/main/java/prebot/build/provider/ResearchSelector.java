...
//				System.out.println("upgradeOrder chk==> " + e.getName());
				...
				if(e.isUpgrade()) {
					if(Prebot.Broodwar.self().getUpgradeLevel(e.getUpgradeType()) == 1 || Prebot.Broodwar.self().isUpgrading(e.getUpgradeType())) {
						continue;
					}else {
				...
				if(e.isTech()) {
					if(Prebot.Broodwar.self().hasResearched(e.getTechType()) || Prebot.Broodwar.self().isResearching(e.getTechType())){
						continue;
					}else {
						metaType = e;
						System.out.println("upgradeOrder selected ==> " + metaType.getTechType());
						break;
					}
				}
