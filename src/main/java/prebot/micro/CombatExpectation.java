...
public class CombatExpectation {
	...
	public static int getVulturePower(List<Unit> vultures) {
		...
		for (Unit vulture : vultures) {
			...
			if (Prebot.Broodwar.self().getUpgradeLevel(UpgradeType.Ion_Thrusters) > 0) {
	...
	public static int enemyPower(List<Unit> targets, boolean skipBuilding) {
		...
		for (Unit target : targets) {
			...
			if (target.getType() == UnitType.Zerg_Zergling && Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Anabolic_Synthesis) > 0) {
				enemyPower += BONUS_ANABOLIC_SYNTHESIS;
			} else if (target.getType() == UnitType.Zerg_Hydralisk) {
				if (Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Muscular_Augments) > 0) {
					enemyPower += BONUS_MUSCULAR_AUGMENTS;
				}
				if (Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Grooved_Spines) > 0) {
			...
			} else if (target.getType() == UnitType.Protoss_Zealot && Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Leg_Enhancements) > 0) {
				enemyPower += BONUS_LEG_ENHANCEMENTS;
			} else if (target.getType() == UnitType.Protoss_Dragoon) {
				if (Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Singularity_Charge) > 0) {
	...
	public static int enemyPowerByUnitInfo(List<UnitInfo> targets, boolean skipBuilding) {
		...
		for (UnitInfo targetInfo : targets) {
			...
			if (InformationManager.Instance().enemyRace == Race.Zerg) {
				...
				if (targetInfo.getType() == UnitType.Zerg_Zergling && Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Anabolic_Synthesis) > 0) {
					enemyPower += BONUS_ANABOLIC_SYNTHESIS;
			...
			if (InformationManager.Instance().enemyRace == Race.Protoss) {
				if (targetInfo.getType() == UnitType.Protoss_Zealot && Prebot.Broodwar.enemy().getUpgradeLevel(UpgradeType.Leg_Enhancements) > 0) {
					enemyPower += BONUS_LEG_ENHANCEMENTS;
