...
public class OldCombatManager {
	...
	public OldSquadData squadData = new OldSquadData();
...
//		MyBotModule.Broodwar.sendText("detailStrategy enabled : " + detailStrategy.toString());
		detailStrategyExFrame[detailStrategy.ordinal()] = Prebot.Broodwar.getFrameCount() + frameDuration;
	}
	public void updateDetailStrategy() {
		for (int i = 0; i < detailStrategyExFrame.length; i++) {
			...
			if((Prebot.Broodwar.getFrameCount() > expireFrame)) {
//				MyBotModule.Broodwar.sendText("detailStrategy disabled : " + CombatStrategyDetail.values()[i].toString());
	...
	public int FastZerglingsInOurBase;
	public int FastZealotInOurBase;
	public int ScoutDefenseNeededMin;
	
	private static OldCombatManager instance = new OldCombatManager();
	
	private OldCombatManager() {
	...
	public static OldCombatManager Instance() {
		return instance;
	}
	...
	public void initSquads() {
		...
		OldSquadOrder idleOrder = new OldSquadOrder(SquadOrderType.IDLE, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.IDLE_RADIUS, "Chill out");
		squadData.putSquad(new OldSquad(OldSquadName.IDLE, idleOrder, Combat.IDLE_PRIORITY));
		
		OldSquadOrder enemyScoutDefense = new OldSquadOrder(SquadOrderType.DEFEND, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.SCOUT_DEFENSE_RADIUS, "Get the scout");
		squadData.putSquad(new OldSquad(OldSquadName.SCOUT_DEFENSE, enemyScoutDefense, Combat.SCOUT_DEFENSE_PRIORITY));
		
		OldSquadOrder EarlyDefense = new OldSquadOrder(SquadOrderType.DEFEND, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.SCOUT_DEFENSE_RADIUS, "Get enemy");
		squadData.putSquad(new OldSquad("EarlyDefense", EarlyDefense, Combat.EARLY_DEFENSE_PRIORITY));
		
		OldSquadOrder attackOrder = new OldSquadOrder(SquadOrderType.ATTACK, getMainAttackLocation(null), Combat.ATTACK_RADIUS, "Attack enemy base");
		squadData.putSquad(new OldSquad(OldSquadName.MAIN_ATTACK, attackOrder, Combat.ATTACK_PRIORITY));
...
//		squadData.putSquad(new Squad("Watcher", watcherOrder, Combat.WATCHER_PRIORITY));
		...
		OldSquadOrder checkerOrder = new OldSquadOrder(SquadOrderType.CHECK, getAttackPosition(null), Combat.CHECKER_RADIUS, "Check it out");
		squadData.putSquad(new OldSquad(OldSquadName.CHECKER, checkerOrder, Combat.CHECKER_PRIORITY));
...
//		squadData.putSquad(new Squad(SquadName.MARINE, marineOrder, Combat.MARINE_PRIORITY));
		...
		OldSquadOrder wraithOrder = new OldSquadOrder(SquadOrderType.ATTACK, getAttackPosition(null), Combat.WRAITH_RADIUS, "Wraith");
		squadData.putSquad(new OldSquad(OldSquadName.WRAITH, wraithOrder, Combat.WRAITH_PRIORITY));
		
		OldSquadOrder vesselOrder = new OldSquadOrder(SquadOrderType.DEFEND, getAttackPosition(null), Combat.VESSEL_RADIUS, "Vessel");
		squadData.putSquad(new OldSquad(OldSquadName.VESSEL, vesselOrder, Combat.VESSEL_PRIORITY));
		
		OldSquadOrder buildingOrder = new OldSquadOrder(SquadOrderType.DEFEND, getAttackPosition(null), Combat.VESSEL_RADIUS, "Building");
		squadData.putSquad(new OldSquad(OldSquadName.BUILDING, buildingOrder, Combat.VESSEL_PRIORITY));
...
//		System.out.println(scvcount1 + ", " + scvcount2);
		...
		if(ScoutDefenseNeeded){
			for (Unit unit : Prebot.Broodwar.self().getUnits()) {
				if((unit.getType() == UnitType.Terran_Marine || unit.getType() == UnitType.Terran_Vulture) && unit.isCompleted()){
					...
					OldSquad scoutDefenseSquad = squadData.getSquad(OldSquadName.SCOUT_DEFENSE);
		...
		if (TimeUtils.executeRotation(0, 7)) {
			...
			if(Prebot.Broodwar.getFrameCount() < 11000){
				updateBunker();
			}
		...
		if (TimeUtils.executeRotation(0, 47)) {
			doComsatScan();
		}
	...
	private void updateBunker() {
		...
		List<Unit> myUnits = Prebot.Broodwar.self().getUnits();
	...
	private void doComsatScan() {
		if (Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Comsat_Station) == 0) {
			return;
		}
...
//		}
	...
	public Position getMainAttackLocation(OldSquad squad) {
...
//			System.out.println("pos2 : " + pos2);
			...
			if (MicroConfig.Common.versusMechanicSet() && tankCount + goliathCount >= MicroConfig.Common.DEFENSE_READY_TO_ATTACK_SIZE_TERRAN) {
				mainAttackLocation = InformationManager.Instance().getReadyToAttackPosition(InformationManager.Instance().selfPlayer);
				
			} else if (tankCount + goliathCount >= MicroConfig.Common.DEFENSE_READY_TO_ATTACK_SIZE) { // 병력이 모였으면 좀더 전진된 위치(정찰, 확장, 공격 용이)
				mainAttackLocation = InformationManager.Instance().getReadyToAttackPosition(InformationManager.Instance().selfPlayer);
				
			} else if (tankCount + goliathCount >= MicroConfig.Common.DEFENSE_SECONDCHOKE_SIZE || InformationManager.Instance().enemyRace == Race.Terran) {
...
//			
	...
	private Position getDefensePosition(OldSquad squad) {
	...
	private Position getAttackPosition(OldSquad squad) {
		// 공격포지션
		if (squad != null) {
			boolean enemyBaseDestoryed = false;
			BaseLocation enemyBaseLocation = InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().enemyPlayer);
			if (Prebot.Broodwar.isVisible(enemyBaseLocation.getTilePosition())) {
		...
		// 적 건물
		Map<Integer, UnitInfo> unitAndUnitInfoMap = InformationManager.Instance().getUnitAndUnitInfoMap(Prebot.Broodwar.enemy());
		...
		// 적 유닛
		for (Unit unit : Prebot.Broodwar.enemy().getUnits()) {
			if (unit.getType() == UnitType.Zerg_Larva || !UnitUtils.isValidUnit(unit, false, true) || !unit.isVisible()) {
				continue;
			}
	...
	public Position letsFindRat() {
		...
		for (BaseLocation base : bases) {
			if (!Prebot.Broodwar.isExplored(base.getTilePosition())) {
	...
	private Position getNextChokePosition(OldSquad squad) {
		BaseLocation enemyBaseLocation = InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().enemyPlayer);
		Chokepoint enemyFirstChoke = InformationManager.Instance().getFirstChokePoint(InformationManager.Instance().enemyPlayer);
	    
		if (squad.getName().equals(OldSquadName.MAIN_ATTACK) && enemyBaseLocation != null && enemyFirstChoke != null) {
		    ...
		    if (nextChokeFind) {
		    	...
		    	int chokeToEnemyChoke = PositionUtils.getGroundDistance(currTargetChoke.getCenter(), enemyFirstChokePosition); // 현재chokepoint ~ 목적지chokepoint
		    	...
		    	for (Chokepoint choke : BWTA.getChokepoints()) {
		    		...
		    		int chokeToNextChoke = PositionUtils.getGroundDistance(currTargetChoke.getCenter(), choke.getCenter()); // 현재chokepoint ~ 다음chokepoint
	...
	private void updateIdleSquad() {
		OldSquad squad = squadData.getSquad(OldSquadName.IDLE);
...
//		System.out.println("bestDist: " + bestDist);
		if(bestTarget!=null){
			if(photonRush){
				...
				for (Unit workerDefender : Prebot.Broodwar.self().getUnits()){
				...
				if(bestDist < 250){
					...
					for (Unit workerDefender : Prebot.Broodwar.self().getUnits()){
						...
						if(gasRush){
							...
							int marine = Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Marine);
							int bunker = Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Bunker);
...
//					doNotAssignTankNGoliath = true;
			...
			if (combatStrategy == CombatStrategy.DEFENCE_INSIDE || combatStrategy == CombatStrategy.DEFENCE_CHOKEPOINT) {
				...
				if (tankCount + goliathCount < MicroConfig.Common.DEFENSE_READY_TO_ATTACK_SIZE && expansion != null && regionCenter.equals(expansion.getRegion().getCenter())) {
...
//	        }
	        ...
	        String squadName = OldSquadName.BASE_DEFENSE_ + regionCenter.getX() + " " + regionCenter.getY();
        	OldSquad squad = squadData.getSquad(squadName);
	        if (enemyUnitsInRegion.isEmpty()) {
	        	...
	        	if (squad == null) {
	        		OldSquadOrder squadOrder = new OldSquadOrder(SquadOrderType.DEFEND, regionCenter, Combat.BASE_DEFENSE_RADIUS, "Defend region");
	        		squadData.putSquad(new OldSquad(squadName, squadOrder, Combat.BASE_DEFENSE_PRIORITY));
	        	}
	        }
...
//			int numZerglingsInOurBase = 0;
			for (Unit unit : enemyUnitsInRegion) {
				...
				if((Prebot.Broodwar.getFrameCount() < 10000 && Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Vulture) <= 2)){
			...
			OldSquad defenseSquad = squadData.getSquad(squadName);
...
//			}
	...
	private void updateDefenseSquadUnits(OldSquad defenseSquad, int flyingDefendersNeeded, int groundDefendersNeeded, boolean pullWorkers, boolean doNotAssignMechanicUnit, boolean doNotAssignTankNGoliath) {
	...
	private void updateAttackSquads() {
	    OldSquad mainAttackSquad = squadData.getSquad(OldSquadName.MAIN_ATTACK);
...
//		int bonusRadius = (int) (Math.log(mainAttackSquad.getUnitSet().size()) * 15);
		OldSquadOrder mainAttackOrder = new OldSquadOrder(SquadOrderType.ATTACK, getMainAttackLocation(mainAttackSquad), Combat.ATTACK_RADIUS, "Attack enemy base");
	    mainAttackSquad.setOrder(mainAttackOrder);
	}
	
	private Unit findClosestDefender(OldSquad defenseSquad, Position pos, boolean flyingDefender, boolean pullWorkers, boolean doNotAssignMechanicUnit, boolean doNotAssignTankNGoliath) {
	...
	private void updateWraithSquad() {//TODO 현재는 본진 공격 용 레이스만 있음
		OldSquad wraithSquad = squadData.getSquad(OldSquadName.WRAITH);
		...
		if(InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().enemyPlayer) != null){
			OldSquadOrder wraithOrder = new OldSquadOrder(SquadOrderType.ATTACK, InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().enemyPlayer).getPoint()
					, UnitType.Terran_Wraith.sightRange(), OldSquadName.WRAITH);
			wraithSquad.setOrder(wraithOrder);
		}
	}
	
	private void updateVesselSquad() {//TODO 현재는 본진 공격 용 레이스만 있음
		OldSquad vesselSquad = squadData.getSquad(OldSquadName.VESSEL);
		for (Unit unit : Prebot.Broodwar.self().getUnits()) {
			if (!UnitUtils.isValidUnit(unit)) {
				continue;
			}
		...
		OldSquadOrder vesselOrder = new OldSquadOrder(SquadOrderType.DEFEND, getMainAttackLocation(vesselSquad), UnitType.Terran_Science_Vessel.sightRange(), "Vessel");
	...
	private void updateBuildingSquad() {
		OldSquad buildingSquad = squadData.getSquad(OldSquadName.BUILDING);
		for (Unit unit : Prebot.Broodwar.self().getUnits()) {
			if (!UnitUtils.isValidUnit(unit)) {
				continue;
			}
	        if (Prebot.Broodwar.getFrameCount() > 12000 && unit.isLifted() && (unit.getType() == UnitType.Terran_Engineering_Bay || unit.getType() == UnitType.Terran_Barracks) && squadData.canAssignUnitToSquad(unit, buildingSquad)) {
		...
		Position sight = null;
		if(temp ==null){
			sight = getMainAttackLocation(squadData.getSquad(OldSquadName.MAIN_ATTACK));
		}else{
			sight = temp.getPosition();
		}
		// ############ 이 아래의 코드는 필요가 없다.(또는 default order로 사용가능) microbuilding실행시 각 유닛의 order로 대체한다. ###############
		OldSquadOrder buildingOrder = new OldSquadOrder(SquadOrderType.DEFEND, sight, UnitType.Terran_Science_Vessel.sightRange(), "Building");
...
//	}
	...
	private void updateCheckerSquad() {
		...
		OldSquad checkerSquad = squadData.getSquad(OldSquadName.CHECKER);
...
//		}
		...
		OldSquadOrder checkerOrder = new OldSquadOrder(SquadOrderType.CHECK, getMainAttackLocation(checkerSquad), Combat.CHECKER_RADIUS, "Check it out");
	...
	private void updateGuerillaSquad() {
		...
		for (Unit unit : combatUnits) {
			if (unit.getType() == UnitType.Terran_Vulture) {
				OldSquad unitSquad = squadData.getUnitSquad(unit);
		...
		if (bestGuerillaSite != null) {
			...
			if (vulturePower > enemyPower) {
				String squadName = OldSquadName.GUERILLA_ + bestGuerillaSite.getPosition().toString();
		...
		// 1. 게릴라 지역이 clear 되었다.
		// 2. 현재 병력으로 이길수 없다.
		List<OldSquad> guerillaSquads = squadData.getSquadList(OldSquadName.GUERILLA_);
		for (OldSquad squad : guerillaSquads) {
			if (squad.getUnitSet().isEmpty()) {
				continue;
			}
