...
import prebot.micro.old.control.MicroBuilding;
import prebot.micro.old.control.MicroGoliath;
import prebot.micro.old.control.MicroMarine;
import prebot.micro.old.control.MicroScv;
import prebot.micro.old.control.MicroTank;
...
public class OldSquad {
	...
	public OldSquad(String name, OldSquadOrder order, int priority) {
		this.name = name;
		this.order = order;
...
//	private Map<Integer, Boolean> nearEnemy = new HashMap<>();
	...
	public OldSquadOrder getOrder() {
		return order;
	}
	public void setOrder(OldSquadOrder order) {
		this.order = order;
	}
	...
	public void update() {
		...
		if (name.equals(OldSquadName.MAIN_ATTACK)) {
			updateMainAttackSquad();
			return;
		} else if (name.equals(OldSquadName.CHECKER) || name.startsWith(OldSquadName.GUERILLA_)) {
			updateVultureSquad();
			return;
		} else if (name.startsWith(OldSquadName.BASE_DEFENSE_)) {
			updateDefenseSquad();
			return;
		} else if (name.equals(OldSquadName.IDLE)) {
			return;
		}
		...
		if (microVulture.getUnits().size() > 0
			...
			Prebot.Broodwar.sendText("!!!!!! MECHANIC UNIT ASSIGN ERROR !!!!!! 010 2626 9786");
		}
	...
	private void updateMainAttackSquad() {
		...
		// 명령 준비
		OldSquadOrder attackerOrder = this.order;
		OldSquadOrder watchOrder = null;
		...
		if (enemyBase != null) {
			...
			watchPosition = OldCombatManager.Instance().letsFindRat();
		}
		watchOrder = new OldSquadOrder(SquadOrderType.WATCH, watchPosition, Combat.WATCHER_RADIUS, "Watch over");
		// WATCHER'S ENEMY
		...
		if (OldCombatManager.Instance().getCombatStrategy() != CombatStrategy.DEFENCE_INSIDE && OldCombatManager.Instance().getCombatStrategy() != CombatStrategy.DEFENCE_CHOKEPOINT) {
...
//				MyBotModule.Broodwar.sendText("finished");
			...
			if (initFrame == 0) {
				...
				if (timeToInitiate) {
					initFrame = Prebot.Broodwar.getFrameCount();
...
//			}
			...
			if (OldCombatManager.Instance().getDetailStrategyFrame(CombatStrategyDetail.VULTURE_JOIN_SQUAD) > 0) { // 후퇴명령
...
//				}
				...
				if (result == Result.LOSS) {
					OldCombatManager.Instance().setDetailStrategy(CombatStrategyDetail.VULTURE_JOIN_SQUAD, MicroConfig.Vulture.getVultureJoinSquadFrame(InformationManager.Instance().enemyRace));
					saveUnitLevelVulture = 2;
				} else if (result == Result.WIN) {
					saveUnitLevelVulture = 1;
				} else if (result == Result.BIG_WIN) {
...
//				System.out.println("keep in line");
		...
		if (OldCombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY
				&& OldCombatManager.Instance().getDetailStrategyFrame(CombatStrategyDetail.ATTACK_NO_MERCY) > 0) { // strategy manager 판단
			saveUnitLevelVulture = saveUnitLevelTank = saveUnitLevelGoliath = 0;
		} else if (InformationManager.Instance().enemyRace != Race.Terran && Prebot.Broodwar.self().supplyUsed() >= 360) { // combat manager 자체 판단
			saveUnitLevelVulture = saveUnitLevelTank = saveUnitLevelGoliath = 0;
		} else if (InformationManager.Instance().enemyRace == Race.Terran && OldCombatManager.Instance().pushSiegeLine) {
			saveUnitLevelVulture = saveUnitLevelTank = saveUnitLevelGoliath = 0;
		}
...
//		return enemyNear.size() > 0;
	...
	public void addUnitsToMicroManagers() {
		...
		for (Unit unit : unitSet) {
			if (!UnitUtils.isValidUnit(unit) || unit.isLoaded()) {
				continue;
			}
