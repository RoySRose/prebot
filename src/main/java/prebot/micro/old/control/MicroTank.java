...
//	        }
			...
			if (tank.isSieged()) {
				...
				if (target == null) {
				...
				} else if (tank.getDistance(target) < MicroConfig.Tank.SIEGE_MODE_MIN_RANGE) {
					// 2. 유효범위내에 적이 없고, 질럿, 저글링 따위가 붙어있다
					tank.unsiege();
					
				} else if (tank.getDistance(target) > MicroConfig.Tank.SIEGE_MODE_MAX_RANGE) {
					// 3. 유효범위내에 적이 없고, 적이 최대범위보다 멀리 있다
					...
					while (closestTank != null && targetIsFree) { // 연결된 시즈 중 타겟에 닿는 시즈가 있는지 체크한다.
						List<Unit> linkedTanks = MapGrid.Instance().getUnitsNear(closestTank.getPosition(), MicroConfig.Tank.SIEGE_LINK_DISTANCE, true, false, UnitType.Terran_Siege_Tank_Siege_Mode);
						closestTank = null;
						for (Unit linkedTank : linkedTanks) {
							int targetDistanceWithLinked = linkedTank.getDistance(target);
							if (targetDistanceWithLinked <= MicroConfig.Tank.SIEGE_MODE_MAX_RANGE) { // 연결된 탱크가 포격할 수 있다면 target은 얻어맞고 시즈를 풀 필요없다.
								targetIsFree = false;
								break;
					...
					if (targetIsFree && Prebot.Broodwar.getFrameCount() - initFrame > 8) {
						tank.unsiege();
					}
//					if (targetIsFree) {
...
//					}
					...
					// 4. 유효범위내에 적이 있다
					CommandUtils.attackUnit(tank, target);
				}
				
			} else { // ***** 탱크모드 *****
				List<Unit> targetsInTankRange = MapGrid.Instance().getUnitsNear(tank.getPosition(), MicroConfig.Tank.TANK_MODE_RANGE, false, true, null);
				if (!MicroConfig.Upgrade.hasResearched(TechType.Tank_Siege_Mode)) {
					// 0. 시즈 개발이 안됐으면 퉁퉁
					Unit target = null;
					...
					if (!targetsInSiegeRange.isEmpty()) {
						...
						List<Unit> linkedTanks = MapGrid.Instance().getUnitsNear(tank.getPosition(), MicroConfig.Tank.SIEGE_LINK_DISTANCE, true, false, UnitType.Terran_Siege_Tank_Siege_Mode);
						for (Unit linkedTank : linkedTanks) {
							List<Unit> targetsInLinkedSiegeRange = MapGrid.Instance().getUnitsNear(linkedTank.getPosition(), MicroConfig.Tank.SIEGE_MODE_MAX_RANGE, false, true, null);
	...
	private Unit getTarget(Unit tank, List<Unit> targets, boolean isSieged) {
		...
		if (isSieged) {
		    ...
		    for (Unit target : targets) {
		        ...
		        if (targetDistance >= MicroConfig.Tank.SIEGE_MODE_MIN_RANGE && targetDistance <= MicroConfig.Tank.SIEGE_MODE_MAX_RANGE) {
	    ...
	    for (Unit target : newTargets) {
	        ...
	        int priorityScore = OldTargetPriority.getPriority(tank, target); // 유닛별 우선순위
	        ...
	        // 스플래시
			if (isSieged) {
				List<Unit> unitsInSplash = target.getUnitsInRadius(MicroConfig.Tank.SIEGE_MODE_OUTER_SPLASH_RAD);
		        for (Unit unitInSplash : unitsInSplash) {
	        		int splashUnitDistance = target.getDistance(unitInSplash);
		        	int priorityInSpash = OldTargetPriority.getPriority(tank, unitInSplash);
