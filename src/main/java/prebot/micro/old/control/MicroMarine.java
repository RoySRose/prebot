...
public class MicroMarine extends MicroManager {
	...
	protected void executeMicro(List<Unit> targets) {
		...
		// 벙커가 없는 경우 행동
		if (bunker == null) {
			Position mineralpos = OldCombatManager.Instance().getBestPosition(CC);
			kitingOption.setGoalPosition(mineralpos);

			for (Unit marine : marines) {
				if (!TimeUtils.executeUnitRotation(marine, LagObserver.groupsize())) {
					continue;
				}
				...
				if (target != null) {
					...
					// if we're not near the order position, go there
						...
						CommandUtils.move(marine, InformationManager.Instance().getSecondChokePoint(InformationManager.Instance().selfPlayer).getPoint());
					}
				}
			}
		} else {// 벙커가 있으면
			...
			for (Unit marine : marines) {
				...
				if (target != null) {
					...
					if (marine.getDistance(bunkerpos) > 30) {
						CommandUtils.move(marine, bunkerpos);
					}
				}
			}
		}
	...
	private Unit getTarget(Unit marine, List<Unit> targets) {
		...
		for (Unit target : targets) {
			...
			int priorityScore = OldTargetPriority.getPriority(marine, target); // 우선순위 점수
	...
	public static Position getFleePosition(Unit rangedUnit, Position targetPosition, int moveDistPerSec,
		...
		// 낑김)
		Integer[] FLEE_ANGLE = fleeAngle != null ? fleeAngle : MicroConfig.FleeAngle.getFleeAngle(rangedUnit.getType());
		...
		while (safePosition == null && moveCalcSize > 10) {
			...
			if (safePosition == null) { // 회피지역이 없을 경우 1) 회피거리 짧게 잡고 다시 조회
				...
				if (moveCalcSize <= 10 && FLEE_ANGLE.equals(MicroConfig.FleeAngle.NARROW_ANGLE)) { // 회피지역이
																								// 없을
																								// 경우
																								// 2)
																								// 각
