...
public class MechanicMicroDecision {
	...
	public static MechanicMicroDecision makeDecisionForSiegeMode(Unit mechanicUnit, List<UnitInfo> enemiesInfo, List<Unit> tanks, OldSquadOrder order, int saveUnitLevel) {
...
//						System.out.println(distanceToTarget + ", " + (MicroSet.Tank.SIEGE_MODE_MAX_RANGE + (int) MicroSet.Common.BACKOFF_DIST_SIEGE_TANK));
			...
			// 우선순위 점수 : 유닛 우선순위 맵
			int priorityScore = OldTargetPriority.getPriority(mechanicUnit.getType(), enemyUnitType);
	        ...
	        for (Unit unitInSplash : unitsInSplash) {
	        	...
	        	int priorityInSpash = OldTargetPriority.getPriority(mechanicUnit, unitInSplash);
				...
				if (enemyUnitType == UnitType.Zerg_Sunken_Colony
					...
					if (enemyUnitType == UnitType.Terran_Siege_Tank_Tank_Mode || enemyUnitType == UnitType.Terran_Siege_Tank_Siege_Mode) {
						...
						if (mechanicUnit.getType() == UnitType.Terran_Siege_Tank_Siege_Mode || mechanicUnit.getType() == UnitType.Terran_Siege_Tank_Tank_Mode) {
							safeDistance += (MicroConfig.Common.BACKOFF_DIST_DEF_TOWER * 2/3);
						} else {
...
//					}
			...
			if (enemy != null) {
				...
				if (enemyUnitType.groundWeapon().maxRange() <= MicroConfig.Tank.SIEGE_MODE_MIN_RANGE) {
					List<Unit> nearSiege = MapGrid.Instance().getUnitsNear(enemyPosition, MicroConfig.Tank.SIEGE_MODE_MIN_RANGE, true, false, UnitType.Terran_Siege_Tank_Siege_Mode);
