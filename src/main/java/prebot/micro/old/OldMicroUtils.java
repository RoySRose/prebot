...
public class OldMicroUtils {
	...
	public static Unit getUnitIfVisible(UnitInfo enemyInfo) {
		Unit enemy = Prebot.Broodwar.getUnit(enemyInfo.getUnitID()); // 보이는 적에 대한 조건
	...
	public static boolean isSafePlace(Unit rangedUnit) {
		boolean isSafe = true;
		List<UnitInfo> nearEnemisUnitInfos = InformationManager.Instance().getNearbyForce(rangedUnit.getPosition(), InformationManager.Instance().enemyPlayer, MicroConfig.Tank.SIEGE_MODE_MAX_RANGE);
		
		for (UnitInfo ui : nearEnemisUnitInfos) {
			Unit unit = Prebot.Broodwar.getUnit(ui.getUnitID()); // 보이는 적에 대한 조건
			...
			if (ui.getType().isWorker() || Prebot.Broodwar.getDamageFrom(ui.getType(), rangedUnit.getType()) == 0) {
				continue;
			}
...
//		    }
	...
	public static void preciseKiting(Unit rangedUnit, Unit target, OldKitingOption kitingOption) {
		...
		if (rangedUnit.getPlayer() != Prebot.Broodwar.self() ||
				!UnitUtils.isValidUnit(rangedUnit) ||
				!UnitUtils.isValidUnit(target, false, false)) {
//			MyBotModule.Broodwar.sendText("smartKiteTarget : bad arg");
		...
		if ((!rangedUnit.isUnderAttack() && target.getType().isWorker())
		...
		} else if (rangedUnit.getType() != UnitType.Terran_Vulture && Prebot.Broodwar.self().weaponMaxRange(rangedUnitWeapon) <= Prebot.Broodwar.enemy().weaponMaxRange(targetWeapon)) {
			// 건물 또는 보다 긴 사정거리를 가진 적에게 카이팅은 무의미하다.
			haveToAttack = true;
			
		} else {
			double distanceToTarget = rangedUnit.getDistance(target);
			double distanceToAttack = distanceToTarget - Prebot.Broodwar.self().weaponMaxRange(rangedUnitWeapon); // 거리(pixel)
			...
			timeToCatch += MicroConfig.Network.LATENCY * 2; // 후퇴해야 하는 경우, 지연시간을 더하면 도망을 더 늦게갈 수도 있다. if (distanceToAttack > 0) // TODO 조절가능
//			timeToCatch += target.getType().isWorker() ? 12 : 0; // 일꾼에게는 좀 덜 도망가도 된다.
		...
		// 공격 또는 회피
		if (haveToAttack) {
			CommandUtils.attackUnit(rangedUnit, target);
...
//		Integer[] FLEE_ANGLE = MicroSet.FleeAngle.getFleeAngle(rangedUnit.getType()); // MicroData.FleeAngle에 저장된 유닛타입에 따른 회피 각 범위(골리앗 새끼들은 뚱뚱해서 각이 넓으면 지들끼리 낑김)
		Integer[] FLEE_ANGLE = fleeAngle != null ? MicroConfig.FleeAngle.getLagImproveAngle(fleeAngle) : MicroConfig.FleeAngle.getFleeAngle(rangedUnit.getType());
		double fleeRadianAdjust = fleeRadian; // 회피 각(radian)
		int moveCalcSize = moveDistPerSec; // 이동 회피지점의 거리 = 유닛의 초당이동거리
		int riskRadius = MicroConfig.RiskRadius.getRiskRadius(rangedUnit.getType());
			...
			for(int i = 0 ; i< FLEE_ANGLE.length; i ++) {
				...
				// 회피지점은 유효하고, 걸어다닐 수 있어야 하고, 안전해야 하고 등등
 				if (isValidGroundPosition(movePosition)
						...
						&& Prebot.Broodwar.hasPath(rangedUnit.getPosition(), movePosition)
...
//		    	riskRadius = riskRadius * 2 / 3;
		    	...
		    	if (moveCalcSize <= 10 && FLEE_ANGLE.equals(MicroConfig.FleeAngle.NARROW_ANGLE)) { // 회피지역이 없을 경우 2) 각 범위를 넓힘
					FLEE_ANGLE = MicroConfig.FleeAngle.WIDE_ANGLE;
					unitedKiting = false;
...
//		MyBotModule.Broodwar.drawCircleMap(safePosition, 5, Color.Red, true);
	...
	public static int riskOfFleePosition(UnitType unitType, Position position, int radius, boolean united) {
		...
		for (Unit u : unitsInRadius) {
			if (u.getPlayer() == InformationManager.Instance().enemyPlayer) { // 적군인 경우
				if (Prebot.Broodwar.getDamageFrom(u.getType(), unitType) > 0) { // 적군이 공격할 수 있으면 위험하겠지
	...
	public static Position vultureRegroupPosition(List<Unit> vultureList, int radius) {
		Position centerOfUnits = OldMicroUtils.centerOfUnits(vultureList);
	    ...
	    for (Chokepoint choke : BWTA.getChokepoints()) {
	    	int risk = OldMicroUtils.riskOfFleePosition(UnitType.Terran_Vulture, choke.getCenter(), radius, true);
	...
	public static boolean killedByNShot(Unit attacker, Unit target, int shot) {
		...
		int damageExpected = Prebot.Broodwar.getDamageFrom(attackerType, targetType, attacker.getPlayer(), target.getPlayer()) * multiply;
	...
	public static boolean isValidGroundPosition(Position position) {
		return position.isValid() && BWTA.getRegion(position) != null && Prebot.Broodwar.isWalkable(position.getX() / 8, position.getY() / 8);
	}
	...
	private static DamageType getDamageType(Unit attacker, Unit target) {
		...
		if (weapon == null || weapon == WeaponType.Unknown) {
			Prebot.Broodwar.sendText("no weapon. no war.");
