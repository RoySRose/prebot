...
import prebot.micro.constant.MicroCode.SquadName;
...
//				MyBotModule.Broodwar.sendText("detailStrategy disabled : " + CombatStrategyDetail.values()[i].toString());
	...
	public void initSquads() {
		...
		SquadOrder idleOrder = new SquadOrder(SquadOrderType.IDLE, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.IDLE_RADIUS, "Chill out");
		squadData.putSquad(new Squad(SquadName.IDLE, idleOrder, Combat.IDLE_PRIORITY));
		
		SquadOrder enemyScoutDefense = new SquadOrder(SquadOrderType.DEFEND, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.SCOUT_DEFENSE_RADIUS, "Get the scout");
		squadData.putSquad(new Squad(SquadName.SCOUT_DEFENSE, enemyScoutDefense, Combat.SCOUT_DEFENSE_PRIORITY));
		
		SquadOrder EarlyDefense = new SquadOrder(SquadOrderType.DEFEND, Prebot.Broodwar.self().getStartLocation().toPosition(), Combat.SCOUT_DEFENSE_RADIUS, "Get enemy");
		...
		if (bestGuerillaSite != null) {
			...
			if (vulturePower > enemyPower) {
				...
				if (guerillaSquad == null) {
	        		SquadOrder squadOrder = new SquadOrder(SquadOrderType.GUERILLA, bestGuerillaSite.getPosition(), MicroConfig.Vulture.GEURILLA_RADIUS, "Let's get it on");
