...
//		handleRepairWorkers();
	...
	// 미네랄 락
	private void handleMineralLockWorkers() {
		...
		for (Unit worker : workerData.getWorkers()) {
			...
			Unit assignMineral = workerData.getWorkerMineralAssignMent(worker);
			Unit realTargetMineral = worker.getOrderTarget();
			if (assignMineral == null || realTargetMineral == null) {
				continue;
			}
			
			if (assignMineral.getID() != realTargetMineral.getID()) {
				worker.gather(assignMineral);
			}
		}	
	}
