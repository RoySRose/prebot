...
public class MarineDefense extends Control {
	...
	@Override
	public void control() {
		Region campRegion = BWTA.getRegion(Idea.of().campPosition);
		...
		// 벙커가 있다면 벙커 주위로 회피
		Position fleePosition = Idea.of().campPosition;
		if (bunker != null) {
		...
		KitingOption kOption = new KitingOption(fOption, true);
		...
		if (bunker == null) {
			for (Unit marine : unitList) {
				...
				if (decision.type == DecisionType.FLEE_FROM_UNIT) {
					...
					CommandUtils.attackMove(marine, Idea.of().campPosition);
				}
			}
	...
	private Unit getCompleteBunker(Region campRegion) {
		...
		for (Unit bunker : UnitUtils.getUnitList(UnitType.Terran_Bunker, UnitFindRange.COMPLETE)) {
	...
	private Unit getIncompleteBunker(Region campRegion) {
		Unit bunkerInRegion = null;
		for (Unit bunker : UnitUtils.getUnitList(UnitType.Terran_Bunker, UnitFindRange.INCOMPLETE)) {
