...
public class TargetScoreCalculator {
	...
	public int calculate(Unit unit, UnitInfo eui) {
		...
		if (enemyUnit != null) {
			...
			// 시즈에 붙어있는 밀리유닛 : +200점
			if (enemyUnitType.groundWeapon().maxRange() <= GameConstant.Tank.SIEGE_MODE_MIN_RANGE) {
				List<Unit> nearSiegeMode = UnitUtils.getUnitsInRadius(enemyPosition, GameConstant.Tank.SIEGE_MODE_MIN_RANGE, PlayerRange.ENEMY, UnitType.Terran_Siege_Tank_Siege_Mode);
			...
			hitPointScore = 50 - eui.getLastHitPoints() / 10;
			specialScore = -100;
