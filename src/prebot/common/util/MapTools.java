...
public class MapTools {
	...
	private static Map<Position, DistanceMap> allMaps = new HashMap<Position, DistanceMap>();
	...
	public static void init() {
		rows = Prebot.Game.mapHeight();
		cols = Prebot.Game.mapWidth();
	...
	public static int getIndex(int row, int col) { // inline
		return row * cols + col;
	}

	public static boolean unexplored(DistanceMap dmap, final int index) {
	...
	public static void setBWAPIMapData() {
		try {
			...
			for (int r = 0; r < rows; ++r) {
				for (int c = 0; c < cols; ++c) {
					...
					for (int i = 0; i < 4; ++i) {
						for (int j = 0; j < 4; ++j) {
							if (!Prebot.Game.isWalkable(c * 4 + i, r * 4 + j)) {
								clear = false;
								break;
							}
	...
	public static void resetFringe() {
		// C+ : std::fill(_fringe.begin(),_fringe.end(),0);
		for (int i = 0; i < fringe.length; i++) {
			fringe[i] = 0;
		}
	...
	private static int getGroundDistance(Position origin, Position destination) {
	...
	public static void computeDistance(DistanceMap dmap, final Position p) {
		search(dmap, p.getY() / 32, p.getX() / 32);
	}

	// does the dynamic programming search
	public static void search(DistanceMap dmap, final int sR, final int sC) {
	...
	public static final Vector<TilePosition> getClosestTilesTo(Position pos) {
		// make sure the distance map is calculated with pos as a destination
		int a = getGroundDistance(Prebot.Game.self().getStartLocation().toPosition(), pos);

		return allMaps.get(pos).getSortedTiles();
	}

	public static TilePosition getTilePosition(int index) {
		return new TilePosition(index % cols, index / cols);
	}

	// 사용하지 않는 API. 지도를 파싱해서 파일로 저장해둔다
	/*
	 * public void parseMap() { //MyBotModule.game.printf("Parsing Map Information"); std::ofstream mapFile; std::string file =
	 * "c:\\scmaps\\" + MyBotModule.game.mapName() + ".txt"; mapFile.open(file.c_str());
	 * 
	 * mapFile << MyBotModule.game.mapWidth()*4 << "\n"; mapFile << MyBotModule.game.mapHeight()*4 << "\n";
	 * 
	 * for (int j=0; j<MyBotModule.game.mapHeight()*4; j++) { for (int i=0; i<MyBotModule.game.mapWidth()*4; i++) { if (MyBotModule.game.isWalkable(i,j)) { mapFile << "0"; } else {
	 * mapFile << "1"; } }
	 * 
	 * mapFile << "\n"; }
	 * 
