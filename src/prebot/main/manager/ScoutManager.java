...
import prebot.xxxdeprecated.MapTools;
...
public class ScoutManager extends GameManager {
	...
	public void update() {
		// 1초에 4번만 실행합니다
		if (PreBot.Broodwar.getFrameCount() % 6 != 0 && InformationManager.Instance().getMainBaseLocation(PreBot.Broodwar.enemy()) != null) {
			return;
		}

		Unit scoutWorker = WorkerManager.Instance().getScoutWorker();
		if (UnitUtils.isValidUnit(scoutWorker)) {
			moveScoutUnit(scoutWorker);
		}
	...
	public void moveScoutUnit(Unit scoutWorker) {
		...
		if (enemyBaseLocation == null) {
			...
			if (currentScoutTargetBaseLocation == null || scoutWorker.getDistance(currentScoutTargetBaseLocation.getPosition()) < 5 * GameConstant.TILE_SIZE) {
				...
				if (closestBaseLocation != null) {
					...
					CommandUtils.move(scoutWorker, closestBaseLocation.getPosition());
		...
		// if we know where the enemy region is
			...
			if (scoutWorker != null) {
				...
				if (PreBot.Broodwar.isExplored(currentScoutTargetBaseLocation.getTilePosition()) == false) {
					...
					CommandUtils.move(scoutWorker, currentScoutTargetPosition);

				} else {
					// currentScoutStatus = ScoutStatus.MoveAroundEnemyBaseLocation.ordinal();
					// currentScoutTargetPosition = getScoutFleePositionFromEnemyRegionVertices(scoutWorker);
					// commandUtil.move(currentScoutUnit, currentScoutTargetPosition);

					WorkerManager.Instance().setIdleWorker(scoutWorker);
	...
	public Position getScoutFleePositionFromEnemyRegionVertices(Unit scoutWorker) {
		...
		if (currentScoutFreeToVertexIndex == -1) {
			...
			int closestPolygonIndex = getClosestVertexIndex(scoutWorker);
		...
		// if we are still fleeing from the previous frame, get the next location if we are close enough
			...
			double distanceFromCurrentVertex = enemyBaseRegionVertices.get(currentScoutFreeToVertexIndex).getDistance(scoutWorker.getPosition());
			...
			while (distanceFromCurrentVertex < 128) {
				...
				distanceFromCurrentVertex = enemyBaseRegionVertices.get(currentScoutFreeToVertexIndex).getDistance(scoutWorker.getPosition());
			}
	...
	public Unit getScoutUnit() {
		return WorkerManager.Instance().getScoutWorker();
	}
