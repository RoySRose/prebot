...
//		saveGameRecordList(isWinner);
	...
	private void executeBuildNewFactory() {
		...
		if (PreBot.Broodwar.self().minerals() > mineralForIncreaseFactory && PreBot.Broodwar.self().gas() > gasForIncreaseFactory) {
			BuildManager.Instance().buildQueue.qHigh(UnitType.Terran_Factory, BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
		}
	}
	...
	private void executeBuildArmory() {
		...
		if (!hasArmory) {
			if (UnitUtils.getUnitCount(UnitType.Terran_Command_Center, UnitFindRange.COMPLETE) >= 2 || PreBot.Broodwar.getFrameCount() > 15000) {
	...
	private void executeBuildAcademyAndComsat() {
		...
		if (hasCompleteAcademy) {
			...
			if (activeCommandCenterCount >= 2 || PreBot.Broodwar.getFrameCount() > 15000) {
...
//					}
	...
	void saveGameRecordList(boolean isWinner) {
		...
		thisGameRecord.myRace = PreBot.Broodwar.self().getRace().toString();
		thisGameRecord.enemyName = enemyName;
		thisGameRecord.enemyRace = PreBot.Broodwar.enemy().getRace().toString();
		thisGameRecord.enemyRealRace = InformationManager.Instance().enemyRace.toString();
		thisGameRecord.gameFrameCount = PreBot.Broodwar.getFrameCount();
		if (isWinner) {
		...
		FileUtil.overwriteToFile(gameRecordFileName, ss.toString());
	}
