...
//		if (Prebot.Game.getFrameCount() % 6 != 0) return;
		...
		/*
		 * System.out.println( "\nCurrent ConstructionTasks in constructionQueue"); for (ConstructionTask b : constructionQueue) { System.out.print(
		 * b.getType() + " status " + b.getStatus() + ", desiredPosition " + b.getDesiredPosition().getX() + "," + b.getDesiredPosition().getY() );
		 * 
		 * if (b.getConstructionWorker() != null) { System.out.println(" constructionWorker " + b.getConstructionWorker().getID() + " isConstructing "
		 * + b.getConstructionWorker().isConstructing()); } else { System.out.println(" constructionWorker is null"); }
		 * 
		 * if (b.getFinalPosition() != null) { System.out.println(" finalPosition " + b.getFinalPosition().getX() + "," +
		 * b.getFinalPosition().getY()); } else { System.out.println(" finalPosition null "); } }
		 */
	...
	public void validateWorkersAndBuildings() {
		...
		for (ConstructionTask b : constructionQueue) {
			if (b.getStatus() == ConstructionTask.ConstructionStatus.UnderConstruction.ordinal()) {
				...
				if (b.getBuildingUnit() == null || !b.getBuildingUnit().getType().isBuilding() || b.getBuildingUnit().getHitPoints() <= 0
						|| !b.getBuildingUnit().exists()) {
	...
	public void assignWorkersToUnassignedBuildings() {
		...
		for (ConstructionTask b : constructionQueue) {
			...
			// System.out.println("assignWorkersToUnassignedBuildings - chooseConstuctionWorkerClosest for " + b.getType() + " to worker near " +
			...
			Unit workerToAssign = WorkerManager.Instance().chooseConstuctionWorkerClosestTo(b.getType(), testLocation, true,
					b.getLastConstructionWorkerID());
	...
	public void constructAssignedBuildings() {
		for (ConstructionTask b : constructionQueue) {
			...
			/*
			 * if (b.getConstructionWorker() == null) { System.out.println( b.getType() + " constructionWorker null" ); } else { System.out.println(
			 * b.getType() + " constructionWorker " + b.getConstructionWorker().getID() + " exists " + b.getConstructionWorker().exists() + " isIdle "
			 * + b.getConstructionWorker().isIdle() + " isConstructing " + b.getConstructionWorker().isConstructing() + " isMorphing " +
		...
		for (Unit buildingThatStartedConstruction : Prebot.Game.self().getUnits()) {
			...
			for (ConstructionTask b : constructionQueue) {
				...
				if (b.getFinalPosition().getX() == buildingThatStartedConstruction.getTilePosition().getX()
					...
					// System.out.println( "Construction " + b.getType() + " started at " + b.getFinalPosition().getX() + "," +
			...
			for (ConstructionTask b : constructionQueue) {
				...
				if (b.getStatus() == ConstructionTask.ConstructionStatus.UnderConstruction.ordinal()) {
					...
					if (b.getConstructionWorker() == null || b.getConstructionWorker().exists() == false
						...
						// System.out.println("checkForDeadTerranBuilders - chooseConstuctionWorkerClosest for " + b.getType() + " to worker near " +
						// b.getFinalPosition().getX() + "," + b.getFinalPosition().getY());

						// grab a worker unit from WorkerManager which is closest to this final position
						Unit workerToAssign = WorkerManager.Instance().chooseConstuctionWorkerClosestTo(b.getType(), b.getFinalPosition(), true,
								b.getLastConstructionWorkerID());
	...
	public void checkForCompletedBuildings() {
		...
		for (ConstructionTask b : constructionQueue) {
			...
			if (b.getBuildingUnit().isCompleted()) {
				// System.out.println("Construction " + b.getType() + " completed at " + b.getFinalPosition().getX() + "," +
				// b.getFinalPosition().getY());
				...
				if (!isDeadlockCase && unitType == UnitType.Terran_Refinery) {
					...
					if (testLocation == TilePosition.None || testLocation == TilePosition.Invalid || testLocation.isValid() == false) {
						...
						if (hasAvailableGeyser == false) {
							System.out.println("Construction Dead lock case . Refinery Building was built already at " + testLocation.getX() + ", "
									+ testLocation.getY());
						}
					}
