...
import prebot.build.InitialBuild;
...
public class StrategyManager {
	...
	private Strategy CurrentStrategyBasic = null;
	private StrategyException CurrentStrategyException = null;
	public Strategy LastStrategyBasic = null;
	public StrategyException LastStrategyException = null;
	...
	public StrategyManager() {
		...
		CurrentStrategyBasic = Strategy.PROTOSSBASIC_CARRIER;
		CurrentStrategyException = StrategyException.INIT;
		setCombatUnitRatio();
	}
	
	public void setCurrentStrategyBasic(Strategy strategy) {
	...
	public void setCurrentStrategyException(StrategyException strategy) {
	...
	public Strategy getCurrentStrategyBasic() {
		return CurrentStrategyBasic;
	}
	public StrategyException getCurrentStrategyException() {
		return CurrentStrategyException;
	}
	
	public Strategy getLastStrategyBasic() {
		return LastStrategyBasic;
	}
	public StrategyException getLastStrategyException() {
...
//		lag.estimate("1");
		...
		if ((Prebot.Broodwar.getFrameCount() < 13000 && Prebot.Broodwar.getFrameCount() % 5 == 0)
				||(Prebot.Broodwar.getFrameCount() >= 13000 && Prebot.Broodwar.getFrameCount() % 23 == 0)) {
			AnalyzeStrategy.Instance().AnalyzeEnemyStrategy();
		}
...
//		lag.estimate("5");
		...
		if (Prebot.Broodwar.getFrameCount() % 31 == 0){// info 의 멀티 체크가 31 에 돈다 
			executeCombat();
		}
//		lag.estimate("6");
...
//		lag.estimate("10");
	...
	private int GetCurrentTot(UnitType checkunit) {
		return BuildManager.Instance().buildQueue.getItemCount(checkunit) + 
				 Prebot.Broodwar.self().allUnitCount(checkunit);
...
//					}
	...
	public void executeAddBuildingInit() {
		...
		if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Factory, null) 
				+ Prebot.Broodwar.self().allUnitCount(UnitType.Terran_Factory)
		...
		if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Refinery, null) 
				+ Prebot.Broodwar.self().allUnitCount(UnitType.Terran_Refinery)
...
//		}
	...
	public void setCombatUnitRatio(){
		...
		if(CurrentStrategyException.toString() == "Init"){
			vultureratio = StrategyConfig.VULTURE_RATIO[CurrentStrategyBasic.ordinal()];
			tankratio = StrategyConfig.TANK_RATIO[CurrentStrategyBasic.ordinal()];
			goliathratio = StrategyConfig.GOLIATH_RATIO[CurrentStrategyBasic.ordinal()];
			wgt = StrategyConfig.WGT[CurrentStrategyBasic.ordinal()];
		}else{
			vultureratio = StrategyConfig.VULTURE_RATIO_EXCEPTION[CurrentStrategyException.ordinal()];
			tankratio = StrategyConfig.TANK_RATIO_EXCEPTION[CurrentStrategyException.ordinal()];
			goliathratio = StrategyConfig.GOLIATH_RATIO_EXCEPTION[CurrentStrategyException.ordinal()];
			wgt = StrategyConfig.WGT_EXCEPTION[CurrentStrategyException.ordinal()];
...
//		}
	...
	public int getTotDeadCombatUnits(){
		...
		int tottank = Prebot.Broodwar.self().deadUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode)
				+Prebot.Broodwar.self().deadUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) ;
	...
	public int getTotKilledCombatUnits(){
		...
		if (InformationManager.Instance().enemyRace == Race.Terran) {
			...
			int totwraith = Prebot.Broodwar.self().deadUnitCount(UnitType.Terran_Wraith);
			int totvessel = Prebot.Broodwar.self().deadUnitCount(UnitType.Terran_Science_Vessel);
			totbio = Prebot.Broodwar.self().killedUnitCount(UnitType.Terran_Marine)
					+ Prebot.Broodwar.self().killedUnitCount(UnitType.Terran_Firebat)
			...
			totlurker = Prebot.Broodwar.self().killedUnitCount(UnitType.Zerg_Lurker) 
					+ Prebot.Broodwar.self().killedUnitCount(UnitType.Zerg_Lurker_Egg);
...
//			tot = MyBotModule.Broodwar.self().killedUnitCount();
	...
	public void executeCombat() {
		...
		if(InformationManager.Instance().enemyRace == Race.Terran){
			int plus=0;
			if( CurrentStrategyBasic == Strategy.TERRANBASIC_BIONIC){
				plus = 2;
			}
			
			if(unitPoint > 10 &&  Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)+Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 4+plus){
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
			}else if(unitPoint > 0 &&  Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)+Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 5+plus){
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
			}else if(Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)+Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 6+plus){
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
			}
			...
			//공통 예외 상황
			if((myunitPoint > 170 || Prebot.Broodwar.self().supplyUsed() > 392 )&& CombatManager.Instance().getCombatStrategy() != CombatStrategy.ATTACK_ENEMY){//팩토리 유닛  130 이상 또는 서플 196 이상 사용시(스타 내부에서는 2배)
			...
			if(totPoint > 120 && CombatManager.Instance().getCombatStrategy() != CombatStrategy.ATTACK_ENEMY && myunitPoint > 80){// 팩토리 유닛이 30마리(즉 스타 인구수 200 일때)
				...
				if (InformationManager.Instance().enemyRace == Race.Zerg && InformationManager.Instance().getNumUnits(UnitType.Zerg_Mutalisk,InformationManager.Instance().enemyPlayer) > 6) {
					...
					Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Goliath)){
						CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
					}
					//triple hatchery
				}else{
			...
			if((LastStrategyBasic == Strategy.PROTOSSBASIC_DOUBLEPHOTO || CurrentStrategyBasic == Strategy.PROTOSSBASIC_DOUBLEPHOTO) 
			...
			if(CombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY){
				...
				if(CombatStartCase == 5){
					...
					if(CombatTime < Prebot.Broodwar.getFrameCount() && ((myunitPoint < 20 && unitPoint <20) || unitPoint < -10)){
						...
						CombatManager.Instance().setCombatStrategy(CombatStrategy.DEFENCE_CHOKEPOINT);
						if(CurrentStrategyBasic == Strategy.PROTOSSBASIC_DOUBLEPHOTO){
	...
	//			}
				...
				if (InformationManager.Instance().enemyRace == Race.Protoss) {
					if(Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Science_Vessel) ==0	&& Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Comsat_Station) ==0){
	...
	public void executeUpgrade() {
		...
		//Fac Unit 30 마리 이상, 일정 이상의 자원 2단계
			...
			if((Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) >= 2 && getFacUnits() > 140)
					||(Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) >= 3 && getFacUnits() > 80)){
	...
	public void executeResearch() {
		...
		boolean VM = (Prebot.Broodwar.self().hasResearched(TechType.Spider_Mines))
				||(Prebot.Broodwar.self().isResearching(TechType.Spider_Mines));
		boolean TS = (Prebot.Broodwar.self().hasResearched(TechType.Tank_Siege_Mode))
				||(Prebot.Broodwar.self().isResearching(TechType.Tank_Siege_Mode));
		...
		if(Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) < 2 
				&& currentResearched >= 2 
				&& getFacUnits() < 32 
				&& !(Prebot.Broodwar.self().minerals()> 300 && Prebot.Broodwar.self().gas()> 250)) return;
...
//		boolean vsProtossbool[] = new boolean[]{VM, VS, TS, GR};
		...
		if(InformationManager.Instance().enemyRace == Race.Protoss){
			...
			if(CurrentStrategyException == StrategyException.PROTOSSEXCEPTION_ZEALOTPUSH
					|| (CurrentStrategyException == StrategyException.INIT
					&& LastStrategyException == StrategyException.PROTOSSEXCEPTION_ZEALOTPUSH)){
...
//			}
			if((CurrentStrategyException == StrategyException.PROTOSSEXCEPTION_DRAGOONPUSH
					|| (CurrentStrategyException == StrategyException.INIT
					&& LastStrategyException == StrategyException.PROTOSSEXCEPTION_DRAGOONPUSH))
				|| (CurrentStrategyException == StrategyException.PROTOSSEXCEPTION_PHOTONRUSH
				|| (CurrentStrategyException == StrategyException.INIT
			    && LastStrategyException == StrategyException.PROTOSSEXCEPTION_PHOTONRUSH))
				){
				Current = vsProtossDragoon;
				Currentbool = vsProtossDragoonbool;
			}
			if(CurrentStrategyException == StrategyException.PROTOSSEXCEPTION_DOUBLENEXUS
					|| (CurrentStrategyException == StrategyException.INIT
					    && LastStrategyException == StrategyException.PROTOSSEXCEPTION_DOUBLENEXUS)){
			...
			if(CurrentStrategyBasic == Strategy.ZERGBASIC_HYDRAWAVE
					|| CurrentStrategyBasic == Strategy.ZERGBASIC_LINGHYDRA
					){
				Current = vsZergHydra;
				Currentbool = vsZergHydrabool;
			}
			
			if(CurrentStrategyException == StrategyException.ZERGEXCEPTION_PREPARELURKER){
				...
				for(int i = 0; i<4; i++){
					if(Currentbool[i] == true){
						...
						if(terranBio && i==2 && Current[2].getTechType() == TechType.Spider_Mines){
							if((getFacUnits() > 48 && Prebot.Broodwar.self().minerals()> 200 && Prebot.Broodwar.self().gas()> 150) || getFacUnits() > 100 ){
	...
	public void executeExpansion() {
		...
		if(RealCCcnt == 1){//TODO 이거 손봐야된다... 만약 위로 띄어서 해야한다면?? 본진에 지어진거 카운트 안되는 상황에서 앞마당에 지어버리겟네
			...
			if((Prebot.Broodwar.self().minerals() > 200 && getFacUnits() > 40 && UnitPoint > 15) || (getFacUnits() > 60 && Attackpoint > 60 && ExpansionPoint > 0)){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
			...
			}if( Prebot.Broodwar.getFrameCount() > 12000 && getFacUnits() > 40){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.FirstExpansionLocation, true);
				}
			}
			if( Prebot.Broodwar.getFrameCount() > 80000 && getFacUnits() > 80){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.FirstExpansionLocation, true);
				}
			}
			if((Prebot.Broodwar.self().minerals() > 600 && getFacUnits() > 40)){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.FirstExpansionLocation, true);
				}
		...
		if(RealCCcnt >= 2){
			...
			if( Prebot.Broodwar.self().minerals() > 600 && getFacUnits() > 120){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
			...
			//공격시 돈 250 넘으면 멀티하기
			if(CombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY && Prebot.Broodwar.self().minerals() > 250){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.NextExpansionPoint, true);
				}
			...
			//500 넘고 유리하면
			if( Prebot.Broodwar.self().minerals() > 500 && getFacUnits() > 50 && Attackpoint > 30){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.NextExpansionPoint, true);
				}
			...
			//200 넘고 유리하면
			if( Prebot.Broodwar.self().minerals() > 200 && getFacUnits() > 80 && Attackpoint > 40 && ExpansionPoint >= 0){
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.NextExpansionPoint, true);
				}
			...
			if(Prebot.Broodwar.getFrameCount() > 14000){
				...
				if( getFacUnits() > 80 
						&& Prebot.Broodwar.self().deadUnitCount() - Prebot.Broodwar.self().deadUnitCount(UnitType.Terran_Vulture)< 12){
					if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
							+ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null)== 0) {
						BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,BuildOrderItem.SeedPositionStrategy.NextExpansionPoint, true);
					}
	...
	private void executeFly() {
		...
		if (Prebot.Broodwar.getFrameCount() > 12000) {
				...
				for (Unit unit : Prebot.Broodwar.self().getUnits()) {
					...
					if (InformationManager.Instance().enemyRace != Race.Zerg) {
						if (unit.getType() == UnitType.Terran_Marine) {
							CommandUtils.move(unit, InformationManager.Instance()
			...
			if (InformationManager.Instance().enemyRace == Race.Terran) {
				...
				Boolean lift = false;
				if(Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Vulture)
...
//						&& MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Vulture) >= 1) {
				...
				if (Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)
						+ Prebot.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 3
						&& dragooncnt + zealotcnt == 0) {
					lift = true;
				}

				if (CurrentStrategyException != StrategyException.PROTOSSEXCEPTION_DRAGOONPUSH
						&& CurrentStrategyException != StrategyException.PROTOSSEXCEPTION_ZEALOTPUSH
						&& CurrentStrategyException != StrategyException.PROTOSSEXCEPTION_READYTOZEALOT
						&& CurrentStrategyException != StrategyException.PROTOSSEXCEPTION_READYTODRAGOON) {
					lift = true;
				}
